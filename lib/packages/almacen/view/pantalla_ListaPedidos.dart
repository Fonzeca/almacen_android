    ...
    return BlocListener<AlmacenBloc,AlmacenState>(
      ...
      BlocBuilder<AlmacenBloc,AlmacenState>(
        builder: (context, state) {
          ...
          if(state.pedidosFiltrados != null){
            pedidos = state.pedidosFiltrados;
          }
          if (pedidos == null){
            return Container();
          }else
            return
              ListView.builder(
                shrinkWrap: true,
                itemCount: pedidos == null ? 1 : pedidos.length + 1,
                itemBuilder: (context, index) {
                  if(admn){
                    ...
                    index -=1;
                    return _createRow(pedidos[index],context,index,admn);
                  }else{
                    if(index==0){
                      return Card(
                          child:Padding(
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) {
        return BlocBuilder<AlmacenBloc, AlmacenState>(
          builder: (context, state) {
            PedidoFiltro filtro = state.filtro;
            return Container(
              height: 500,
              ...
              child: Column(
                ...
                children: [
                  ...
                  CheckboxListTile(
                    title: Text("En Curso"),
                    value: filtro.estados.contains(PedidoEstados.EnCurso),
                    onChanged: (value) {
                      filtro.setFilter(PedidoEstados.EnCurso, value);
                      BlocProvider.of<AlmacenBloc>(context).add(AlmacenEventFiltrarPedidos(filtro));
                    },
                  ),
                  CheckboxListTile(
                    title: Text("En Espera"),
                    value: filtro.estados.contains(PedidoEstados.EnEspera),
                    onChanged: (value) {
                      filtro.setFilter(PedidoEstados.EnEspera, value);
                      BlocProvider.of<AlmacenBloc>(context).add(AlmacenEventFiltrarPedidos(filtro));
                    },
                  ),
                  CheckboxListTile(
                    title: Text("Entregado"),
                    value: filtro.estados.contains(PedidoEstados.Entregado),
                    onChanged: (value) {
                      filtro.setFilter(PedidoEstados.Entregado, value);
                      BlocProvider.of<AlmacenBloc>(context).add(AlmacenEventFiltrarPedidos(filtro));
                    },
                  ),
                ],
